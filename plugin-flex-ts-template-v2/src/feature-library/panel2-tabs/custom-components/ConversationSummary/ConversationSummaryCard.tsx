...
import { WorkerActionPayload } from '@twilio/flex-ui/src/actions/WorkerActions';
import { SegmentTrackData } from 'feature-library/panel2-tabs/types/Segment/SegmentTrackData';
...
const ConversationSummaryCard = (props: CardProps) => {
  ...
  const pushSummaryToSegment = async () => {
    ...
    try {
      if (!props.task?.attributes.customers?.external_id || !props.task?.attributes?.email || !newSummary) {
        throw new Error('Missing properties');
      }

      await SegmentService.sendToSegment({
        actor: 'Flex',
        eventName: 'GPT Summary',
        userId: props.task?.attributes?.email,
        channelType: 'chat',
        email: props.task?.attributes?.email,
        sentiment,
        summary: newSummary,
      } as SegmentTrackData);

      // await SendToSegmentService.push(
      //   props.task?.attributes.customers?.external_id as string,
      //   props.task?.attributes?.email as string,
      //   newSummary as string,
      //   sentiment?.sentiment as string,
      //   props.task?.attributes.currentLanguage || 'en',
      // );
    } catch (error: any) {
      const err = error.message;
      console.error(err);
        ...
        <Grid vertical={[true, true, false]}>
          ...
          <Column span={[6, 6, 3]} offset={1}>
            <PasteFlex vAlignContent="center" marginBottom="space30" hAlignContent="right">
              {summary && !loadingSentiment && !summaryError && <>{sentimentMap[sentiment?.sentiment as string]}</>}
              {loadingSentiment && <Spinner size="sizeIcon30" decorative={true} title="Loading Sentiment" />}
            </PasteFlex>
          </Column>
        </Grid>
