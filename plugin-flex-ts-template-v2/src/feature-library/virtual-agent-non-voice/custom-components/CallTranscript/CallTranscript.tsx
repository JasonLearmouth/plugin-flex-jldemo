...
const CallTranscript: React.FC<CallTranscriptProps> = (props: CallTranscriptProps) => {
  ...
  const BASE_URL = getBackendUrl() || 'https://dialogflow-voice-transcript-service-2920-dev.twil.io';
  const API_TRANSCRIPT = '/transcript/retrieve?CallSid=';

  useEffect(() => {
    if (!props?.task?.attributes.va_call_sid || props?.task?.attributes.va_call_sid == undefined) return;
    console.log(t('log.lookup-callsid-start'), props?.task?.attributes.va_call_sid);

    api<any>(BASE_URL + API_TRANSCRIPT + props?.task?.attributes.va_call_sid, {})
      .then((apiData) => {
  ...
  }, [props?.task?.attributes.va_call_sid]);
      ...
      >
        <ChatLog>
          {transcript && (
            <ChatBookend>
              <ChatBookendItem>
                <strong>{t('transcript.bot-name')}</strong> {t('transcript.transcript-start')}・
                ...
                >
                  {item.data.ResolvedInput && (
                    <ChatMessage variant="inbound">
                      ...
                      <ChatMessageMeta aria-label={t('transcript.said-by-customer')}>
                        <Tooltip text={item.data.EndUserId}>
                          <ChatMessageMetaItem>
                            <Avatar name={t('transcript.customer')} size="sizeIcon20" />
                            {t('transcript.customer')} ・ <Moment format="hh:mm:ss" date={item.dateCreated} />
                          </ChatMessageMetaItem>
                        </Tooltip>
                        {item.data.SentimentAnalysisScore && (
                          <Tooltip text={item.data.SentimentAnalysisScore || 'Unknown'}>
                            <ChatMessageMetaItem>
                              <Badge
                                as="span"
                                variant={getVariantForSentiment(parseFloat(item.data.SentimentAnalysisScore as string))}
                              >
                                {getLabelForSentiment(parseFloat(item.data.SentimentAnalysisScore as string))}
                              </Badge>
                            </ChatMessageMetaItem>
                          </Tooltip>
                        )}
                  ...
                  <ChatMessage variant="outbound">
                    ...
                    <ChatMessageMeta aria-label={t('transcript.said-by-customer')}>
                      <Tooltip text={item.data.IntentDisplayName || item.data.IntentId || t('word.unknown')}>
                        <ChatMessageMetaItem>
                          <Avatar name={t('transcript.bot-name')} size="sizeIcon20" />
                          {t('transcript.bot-name')} ・ <Moment format="hh:mm:ss" date={item.dateCreated} />
                        </ChatMessageMetaItem>
                      </Tooltip>
                    </ChatMessageMeta>
                  </ChatMessage>
          ...
          {transcript && (
            <ChatBookend>
              <ChatBookendItem>
                ...
                <Moment format="hh:mm:ss" date={transcript[transcript.length - 1].dateCreated} />
              </ChatBookendItem>
            </ChatBookend>
          )}
        </ChatLog>
